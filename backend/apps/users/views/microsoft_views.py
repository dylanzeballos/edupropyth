...
class MicrosoftAuthView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.info(f"Attempting to exchange code for token with client_id: {env('MICROSOFT_CLIENT_ID')}")
            
            token_response = requests.post(token_url, data=token_data)
            
            if token_response.status_code != 200:
                logger.error(f"Microsoft token error: {token_response.status_code} - {token_response.text}")
                return Response(
            ...
            if user_response.status_code != 200:
                logger.error(f"Microsoft user info error: {user_response.status_code} - {user_response.text}")
                return Response(
            ...
            logger.info(f"Retrieved user data from Microsoft: {user_data.get('userPrincipalName')}")
            ...
            response_data = {
                ...
                'user': {
                    ...
                    'profile_role': getattr(user.profile, 'role', 'student') if hasattr(user, 'profile') else 'student'
                }
            }
...
def microsoft_callback(request):
    ...
    return Response(
        {"error": "No code or error provided"}, 
        status=status.HTTP_400_BAD_REQUEST
    )